...
use commonware_runtime::{Handle, Metrics};
...
impl TempoNetworkBuilder {
    ...
    pub(crate) fn build(self, context: &Context) -> eyre::Result<TempoNetwork> {
        ...
        let p2p_namespace =
            commonware_utils::union_unique(crate::config::NAMESPACE, b"_P2P");
        let default_config = if use_local_defaults {
            lookup::Config::local(signing_key, &p2p_namespace, listen_address, max_message_size)
        } else {
