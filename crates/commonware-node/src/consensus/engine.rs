...
fn into_duration(d: jiff::SignedDuration) -> eyre::Result<Duration> {
    d.try_into().wrap_err("invalid consensus duration; was it negative or too large?")
}
        ...
        commonware_p2p::Manager<PublicKey = PublicKey, Peers = Map<PublicKey, Address>> + Sync,
{
            ...
            + Network,
    {
        ...
        let time_to_collect_notarizations = self
            .time_to_collect_notarizations
            .ok_or_eyre("time_to_collect_notarizations is required - call with_time_to_collect_notarizations()")?;
        ...
        let time_for_peer_response = self
            .time_for_peer_response
            .ok_or_eyre("time_for_peer_response is required - call with_time_for_peer_response()")?;
        let views_to_track = self
            .views_to_track
            .ok_or_eyre("views_to_track is required - call with_views_to_track()")?;
        let views_until_leader_skip = self
            .views_until_leader_skip
            .ok_or_eyre("views_until_leader_skip is required - call with_views_until_leader_skip()")?;
        ...
        let time_to_build_subblock = self
            .time_to_build_subblock
            .ok_or_eyre("time_to_build_subblock is required - call with_time_to_build_subblock()")?;
        let subblock_broadcast_interval = self
            .subblock_broadcast_interval
            .ok_or_eyre("subblock_broadcast_interval is required - call with_subblock_broadcast_interval()")?;
        let fcu_heartbeat_interval = self
            .fcu_heartbeat_interval
            .ok_or_eyre("fcu_heartbeat_interval is required - call with_fcu_heartbeat_interval()")?;
        let feed_state = self
            .feed_state
